...
export default function CreateTreatmentTypeModal({ isOpen, onClose, onSave }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await api.post('/treatments-types', payload);
      
      onSave(response.data);
      setNotification({ type: "success", message: "Tipo de tratamiento creado correctamente" });
      ...
      console.error(err);
      const msg = err.response?.data?.message || 'Error al crear tipo de tratamiento';
      ...
      <div className="fixed inset-0 flex justify-center items-center z-50 bg-black/10 backdrop-brightness-90">
        ...
        >
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
                <label className="block mb-2 text-sm font-medium">Nombre del Tipo <span className="text-red-600">*</span>:</label>
                <input
                  type="text"
                  name="name"
                  value={formData.name}
                  placeholder="Ej: Ortodoncia, Limpieza..."
